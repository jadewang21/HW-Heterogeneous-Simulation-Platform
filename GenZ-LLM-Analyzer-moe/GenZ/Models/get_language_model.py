...
from GenZ.Models.flash_attention_config import get_flash_attention_implementation
...
def create_inference_moe_prefill_layer(input_sequence_length, name='GPT-2', data_path=DATA_PATH,
    ...
    layers = flash_attention_prefill(model_config, parallelism_config, input_sequence_length) + ffn_prefill(model_config, parallelism_config, input_sequence_length)
    if isinstance(name, ModelConfig):
        name = name.model
...
def create_inference_moe_decode_layer(input_sequence_length, name='GPT-2', data_path=DATA_PATH,
    ...
    # 使用配置模块获取Flash Attention实现
    _, flash_attention_decode = get_flash_attention_implementation()
    layers = flash_attention_decode(model_config, parallelism_config, input_sequence_length, output_gen_tokens) + ffn_decode(model_config, parallelism_config)
    if isinstance(name, ModelConfig):
        name = name.model
    ...
    def add_layers(layers, num_layers):
        ...
        if model_config.unique_layers == 1:
            if model_config.layer_type[0][0] == "MHA-global":
                # 使用配置模块获取Flash Attention实现
                _, flash_attention_decode = get_flash_attention_implementation()
                layers += flash_attention_decode(model_config, parallelism_config, input_sequence_length, output_gen_tokens)
            elif model_config.layer_type[0][0] == "Mamba":
