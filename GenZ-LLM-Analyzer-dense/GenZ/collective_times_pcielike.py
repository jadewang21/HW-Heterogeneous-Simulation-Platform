def get_AR_time(data, numNodes, system):
    ...
    allReduceTime = (6e-5 + 2*(numNodes-1)*system.interchip_link_latency +  2 * (numNodes-1) * (data/numNodes) / system.interchip_link_bw)*1000
...
def get_AG_time(data, numNodes, system):
    ...
    ## Source:  https://tech.preferred.jp/en/blog/technologies-behind-distributed-deep-learning-allreduce/
    allGatherDuration = (6e-5 + (numNodes-1)*system.interchip_link_latency +  (numNodes-1) * (data/numNodes) / system.interchip_link_bw)*1000
...
def get_A2A_time(data, numNodes, system):
    ...
    A2A_time = (6e-5 + (numNodes - 1) * system.interchip_link_latency +
