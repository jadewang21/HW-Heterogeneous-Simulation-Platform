...
# So, its KV cache is equal to GQA with only 2.25 groups, but its performance is stronger than MHA.
    ...
    ## [Batch/dp, Seq, Dq, Head/tp] * [Batch/dp, Seq, Dq, Head/tp] = [Batch/dp, Seq, Seq, Head/tp]
    logit =         [["Logit",per_node_H, input_sequence_length, input_sequence_length, Dq, per_node_Hkv, ResidencyInfo.C_onchip, OpType.Logit]]

    ## [Batch/dp, Seq, Seq, Head/tp] * [Batch/dp, Seq, Dq, Head/tp] = [Batch/dp, Seq, Dq, Head/tp]
...
def mha_flash_attention_chunked(model_config:ModelConfig, parallelism_config:ParallelismConfig,
    ...
    ## Decode LA layers - 修正：确保FLOPs计算基于完整的序列长度
    ## Flash Attention的收益应该通过内存效率系数体现，而不是减少FLOPs
    for kv_size in decode_kv_sizes:
        if isinstance(kv_size, tuple) and len(kv_size) == 4:
            ...
            # 修正：使用完整的序列长度，而不是分块后的维度
            prefill_context = kv_size[2]  # 移除//sp，使用完整序列长度
...
def mha_flash_attention_corrected_prefill(model_config:ModelConfig, parallelism_config:ParallelismConfig, input_sequence_length:int):
    """
    修正后的Flash Attention实现，确保FLOPs计算基于完整的序列长度。
    Flash Attention的收益通过内存效率系数体现，而不是减少FLOPs。
    """
    H = model_config.num_attention_heads
    Hkv = model_config.num_key_value_heads
    D = model_config.hidden_size
...
def mha_flash_attention_corrected_decode(model_config:ModelConfig, parallelism_config:ParallelismConfig, input_sequence_length:int, output_gen_tokens:int):
    """
    修正后的Flash Attention Decode实现，确保FLOPs计算基于完整的序列长度。
    """
    H = model_config.num_attention_heads
    Hkv = model_config.num_key_value_heads
    D = model_config.hidden_size
